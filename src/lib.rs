//! # survex-rs
//! Rust bindings to the Survex `img.c` library which can be used via a safe API utilising the
//! [`SurveyData`][`data::SurveyData`] and [`Station`][`station::Station`] structs, or directly via
//! unsafe Rust. For the source code and to contribute, see the GitHub repository at
//! [anorthall/survex-rs](https://github.com/anorthall/survex-rs).
//!
//! ## Safe API
